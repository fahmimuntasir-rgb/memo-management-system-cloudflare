...
async function notifyUsers(env:Env,organizationId:string,userIds:string[],memoId:string|null,type:string,message:string){
 const now=new Date().toISOString(),ids=[...new Set(userIds.filter(Boolean))];if(!ids.length)return;
 await env.DB.batch(ids.map(id=>env.DB.prepare("INSERT INTO notifications(id,organization_id,user_id,memo_id,type,message,created_at) VALUES(?,?,?,?,?,?,?)").bind(crypto.randomUUID(),organizationId,id,memoId,type,message,now)));
}
async function memoAudience(env:Env,organizationId:string,memoId:string,excludeId?:string){
 const rows=await env.DB.prepare("SELECT author_id user_id FROM memos WHERE id=? AND organization_id=? UNION SELECT participant_id user_id FROM workflow_steps WHERE memo_id=? AND organization_id=?").bind(memoId,organizationId,memoId,organizationId).all<{user_id:string}>();return rows.results.map(x=>x.user_id).filter(id=>id!==excludeId);
}
...
async function v2(req:Request,env:Env,path:string,u:User):Promise<Response|null>{
 ...
 if(action&&req.method==="POST"){
  ...
  if(kind==="Comment"){if(!b.comment?.trim())return json({error:"Comment is required."},400);const now=new Date().toISOString(),memo:any=await env.DB.prepare("SELECT reference_no FROM memos WHERE id=? AND organization_id=?").bind(id,u.organization_id).first();await env.DB.prepare("INSERT INTO comments(id,organization_id,memo_id,user_id,comment_type,text,created_at) VALUES(?,?,?,?,?,?,?)").bind(crypto.randomUUID(),u.organization_id,id,u.id,"General",b.comment.trim(),now).run();await event(env,u,id,"COMMENT_ADDED",u.name+" added a comment",b.comment.trim());await notifyUsers(env,u.organization_id,await memoAudience(env,u.organization_id,id,u.id),id,"COMMENT_ADDED",u.name+" commented on "+memo.reference_no);return json({ok:true})}
  ...
  const now=new Date().toISOString(),memo:any=await env.DB.prepare("SELECT author_id,reference_no FROM memos WHERE id=? AND organization_id=?").bind(id,u.organization_id).first();
  if(kind==="Approve"){
   ...
   await env.DB.batch(stmts);await event(env,u,id,next?"WORKFLOW_ADVANCED":"WORKFLOW_COMPLETED",(next?"Memo moved to the next participant":"Memo approved and completed")+(actingFor?" under delegated authority":""),b.comment||null);if(memo.author_id!==u.id)await notifyUsers(env,u.organization_id,[memo.author_id],id,next?"WORKFLOW_ADVANCED":"MEMO_APPROVED",next?memo.reference_no+" advanced to the next participant":memo.reference_no+" was approved and completed");
  }else{const status=kind==="Reject"?"Rejected":"Changes Requested";await env.DB.batch([env.DB.prepare("UPDATE workflow_steps SET status=?,acted_by=?,acted_on_behalf_of=?,action=?,comment=?,completed_at=? WHERE id=? AND organization_id=?").bind(status,u.id,actingFor,kind,b.comment.trim(),now,step.id,u.organization_id),env.DB.prepare("UPDATE memos SET status=?,updated_at=? WHERE id=? AND organization_id=?").bind(status,now,id,u.organization_id)]);await event(env,u,id,kind==="Reject"?"MEMO_REJECTED":"CHANGES_REQUESTED","Memo "+status.toLowerCase()+(actingFor?" under delegated authority":""),b.comment.trim());if(memo.author_id!==u.id)await notifyUsers(env,u.organization_id,[memo.author_id],id,kind==="Reject"?"MEMO_REJECTED":"CHANGES_REQUESTED",memo.reference_no+(kind==="Reject"?" was rejected":" requires changes"))}
  return json({ok:true});
 }
 ...
 const notificationRead=path.match(/^\/api\/v2\/notifications\/([^/]+)\/read$/);if(notificationRead&&req.method==="POST"){await env.DB.prepare("UPDATE notifications SET read_at=COALESCE(read_at,?) WHERE id=? AND organization_id=? AND user_id=?").bind(new Date().toISOString(),notificationRead[1],u.organization_id,u.id).run();return json({ok:true})}
